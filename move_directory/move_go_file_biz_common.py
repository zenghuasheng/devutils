...
def parse_command_line_args():
    ...
    parser.add_argument("--main-dir", help="directory path of bang-api", required=False,
                        default='/Users/xhs/go/src/github.com/bangwork/ones-api-biz-common')
    ...
    parser.add_argument("--symbol-bin-path", help="the symbol bin file path to extract go symbols, "
                        ...
                        default='/Users/xhs/go/src/github.com/bangwork/ones-api-biz-common-gomod/tmp/extract_symbol')
...
def move_dir(main_dir, go_path, source, target, no_compile=False):
    ...
    # 替换 import
    # find /Users/xhs/go/src/github.com/bangwork/ones-api-biz-common-gomod/app/services/common/organization -type f -name '*.go' -exec sed -i '' 's|"github\.com/bangwork/ones-api-biz-common/app/models/user"|"github\.com/bangwork/ones-api-biz-common/biz-common/user/models/user"|g' {} +
    replace_commands = [
        f"find {main_dir} -type f -name '*.go' -exec sed -i '' 's|\"github\.com/bangwork/ones-api-biz-common/{source}\"|\"github\.com/bangwork/ones-api-biz-common/{target}\"|g' {{}} +",
        f"find {main_dir} -type f -name '*.go' -exec sed -i '' 's|\"github\.com/bangwork/ones-api-biz-common/{source}/|\"github\.com/bangwork/ones-api-biz-common/{target}/|g' {{}} +",
    ]
    ...
    if import_match:
        ...
        for match in import_lines:
            if not match:
                continue
            package = match[0]
            # 'jsonUtils "github.com/bangwork/ones-api-biz-common/app/utils/json"'
...
def search_name(imported_packages, source_package_path):
    ...
    for imported_package in imported_packages:
        ...
        if package == f'github.com/bangwork/ones-api-biz-common/{source_package_path}':
            # 如果别名为空，使用包名
            if not alias:
                alias = package.split("/")[-1].replace('"', '')
            return alias
...
class MoveGo:
    ...
    def search_imported_files(self):
        ...
        # grep -r "github\.com/bangwork/ones-api-biz-common/app/models" --include='*.go' /Users/xhs/go/src/github.com/bangwork/ones-api-biz-common-gomod | cut -d ':' -f 1 | more
        cmd = f'grep -r "\\"github\.com/bangwork/ones-api-biz-common/{self.source_package_path}\\"" --include="*.go" {self.main_dir} | cut -d ":" -f 1'
        if self.debug:
            print(cmd)
        result = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode != 0:
    ...
    def replace(self):
        ...
        # self.imported_files = ['/Users/xhs/go/src/github.com/bangwork/ones-api-biz-common-gomod/app/services/manhour/message.go']
        for file_path in self.imported_files:
            ...
            with open(file_path, 'r+') as f:
                ...
                if set(file_symbols).issubset(set(self.file_exported_symbol_names)):
                    # 替换 import 就行
                    go_code = go_code.replace(f'"github.com/bangwork/ones-api-biz-common/{self.source_package_path}"',
                                              f'"github.com/bangwork/ones-api-biz-common/{self.target_package_path}"')
                    f.write(go_code)
                else:
                    have_symbol = False
                    ...
                    go_code = add_import(go_code,
                                         f'{import_alias} "github.com/bangwork/ones-api-biz-common/{self.target_package_path}"')
                    # 替换
                    for s in self.file_exported_symbol_names:
...
class MoveGoFiles:
    ...
    def replace(self):
        ...
        # 读取文件 A 内容，列出所有 package 的函数，常量
        # self.imported_files = ['/Users/xhs/go/src/github.com/bangwork/ones-api-biz-common-gomod/project-api/task/models/workflow/types.go']
        for file_path in self.imported_files:
            if self.debug:
                print(f'processing {file_path}')
            with open(file_path, 'r+') as f:
