...
# 计算要输出的前TOP%的数据条数
top_percentage = total_count * TOP / 100
...
# 使用正则表达式提取替换后的URL，并统计其数量
urls = re.findall(r'"(GET|POST|HEAD|OPTIONS) ([^\s\"]+)', log_data)
url_counts = Counter(urls)

# 输出URL数量排行
temp_top = top_percentage
print("URL数量排行：")
for url, count in url_counts.most_common():
    print(count, url)
    top_percentage -= count
    # print(top_percentage)
    if top_percentage <= 0:
        break

# 输出总数量
print("总数量：", total_count, "统计数量：", temp_top)
